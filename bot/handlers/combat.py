...
from database import get_user, update_user, get_item, get_loot_table, get_item_name, get_setting, apply_flee_penalty, apply_death_penalty, add_quest_progress
    ...
    if random.random() < mob_skills.get("dodge", 0):
        ...
        if enemy_hp > 0 and random.random() < mob_skills.get("counter", 0):
            ...
            if user['hp'] <= 0:
                ...
                return await callback.message.edit_text("☠️ **Вы убиты!** Все собранные в подземелье вещи утеряны.", reply_markup=get_town_kb(), parse_mode="Markdown")
    
    combat['enemy_hp'] = enemy_hp
    
    if enemy_hp <= 0:
        # ПРОГРЕСС КВЕСТОВ (Убийство)
        add_quest_progress(user['user_id'], "kill_mobs", 1)
        
        dungeon_data = user.get('dungeon_data', {})
        dtype = dungeon_data.get('dungeon_type', 'solo')
        location_id = f"event_{datetime.datetime.today().weekday()}" if dtype == "event" else "solo"
        ...
        for loot in loot_table:
            if random.random() <= loot['chance']:
                ...
                if item_db and item_db['type'] in ["weapon", "armor"]:
                    ...
                    materials[loot['item_id']] = materials.get(loot['item_id'], 0) + qty
        ...
        if not dungeon_data.get("nodes", {}).get(dungeon_data.get("current_node"), {}).get("next"):
            # ПРОГРЕСС КВЕСТОВ (Победа над Боссом)
...
async def combat_use_potion(callback: CallbackQuery):
    ...
    if "рагу" in p_lower:
        ...
        log_msg += f"+{heal} ХП!"
    elif "хил" in p_lower or "реген хп" in p_lower:
        heal = int(user['max_hp'] * 0.25)
        user['hp'] = min(user['max_hp'], user['hp'] + heal)
        log_msg += f"+{heal} ХП!"
    elif "урон огнем" in p_lower or "урон ядом" in p_lower or "сила тьмы" in p_lower:
        dmg = 30 + user.get('level', 1) * 5
        combat['enemy_hp'] = combat.get('enemy_hp', 50) - dmg
        log_msg += f"Враг получает {dmg} урона колдовством!"
    elif "реген од" in p_lower or "ускорение" in p_lower:
        combat['ap'] += 2
        log_msg += "+2 ОД!"
    elif "вампиризм" in p_lower:
        v_dmg = 20
        combat['enemy_hp'] = combat.get('enemy_hp', 50) - v_dmg
        user['hp'] = min(user['max_hp'], user['hp'] + v_dmg)
        log_msg += f"Враг иссушен на {v_dmg} ХП!"
    elif "богатство" in p_lower or "удача" in p_lower:
        ...
        log_msg += f"Прямо в воздухе материализовалось {gold_b} 🪙!"
    else:
        combat['enemy_hp'] = combat.get('enemy_hp', 50) - 10
        log_msg += "Враг ослаблен! (-10 ХП)"
...
async def combat_flee(callback: CallbackQuery):
    lost_gold, lost_mats = apply_flee_penalty(callback.from_user.id, get_user(callback.from_user.id).get('dungeon_data', {}))
