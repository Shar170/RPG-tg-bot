...
from database import get_user, update_user, get_item, get_item_name, get_item_price, get_connection
...
async def market_buy_do(callback: CallbackQuery):
    ...
    # Списание золота и добавление предмета
    user['gold'] -= price
    inv = user['inventory']
    ...
    if item['type'] in ["weapon", "armor"]:
        ...
        # Чтобы зелья корректно отображались в боевом меню, добавляем их по имени
    ...
    # Возвращаемся в ту же категорию, чтобы обновить отображаемый баланс
...
async def market_buy_category(callback: CallbackQuery):
    ...
    buttons = []
    for item in items:
        # Выводим статы для оружия и брони
        stats_str = ""
        ...
        btn_text = f"{item['name']}{stats_str} — {item['price']} 🪙"
...
async def market_sell_loot(callback: CallbackQuery):
    ...
    text = (
        f"💎 **Скупка лута**\n💰 Ваше золото: {user['gold']} 🪙\n\n"
        "Выберите предмет для продажи (Торговец берет всё за **50%** от стоимости):\n"
    )
    buttons = []
    ...
    for mat_id, count in sorted(materials.items()):
        if count > 0:
            ...
            price = max(1, get_item_price(mat_id) // 2)
...
async def market_sell_loot_item(callback: CallbackQuery):
    ...
    name = get_item_name(mat_id)
    price_per_one = max(1, get_item_price(mat_id) // 2)
...
async def market_sell_loot_do(callback: CallbackQuery):
    ...
    # Списываем предметы и начисляем золото
    ...
    # Возврат к списку для дальнейшей продажи
...
async def market_sell_equip(callback: CallbackQuery):
    ...
    text = (
        ...
        "Выберите элемент для продажи (Скупка за **50%**).\n"
        "*(Внимание: при нажатии продается сразу 1 шт.!)*\n"
    )
    buttons = []
    
    # Группируем одинаковые элементы
    ...
    for item_id, count in counts.items():
        ...
        name = item_data['name']
        price = max(1, item_data['base_price'] // 2)
...
async def market_sell_equip_do(callback: CallbackQuery):
    ...
    # Удаляем только одну копию предмета (первое вхождение)
    ...
    # Сразу обновляем список снаряжения
